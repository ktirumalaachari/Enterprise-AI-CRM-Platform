...
export type JoinRequestFilterTab =
  | 'ALL'
  | 'PENDING'
  | 'APPROVED'
  | 'REJECTED';
...
export const CompanySettings = () => {
  ...
  const [activeTab, setActiveTab] =
    useState<JoinRequestFilterTab>('ALL');
  ...
  const [confirmRejectId, setConfirmRejectId] = useState<string | null>(
    null
  );
  ...
  // ---------------------------------------------------------------------------
  // Background Join Request Refresh
  // ---------------------------------------------------------------------------
  ...
  const refreshJoinRequests = useCallback(async () => {
    try {
      ...
      // Background refresh should fail silently.
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const connectSSE = async () => {
      try {
        ...
        if (!token) {
          console.warn(
            '[SSE] Access token unavailable. Polling fallback remains active.'
          );
        ...
        const sseUrl = `${apiBaseUrl.replace(
          /\/$/,
          ''
            ...
            for (const eventBlock of eventBlocks) {
              ...
              try {
                ...
                if (
                  ...
                  info(
                    `New Team Join Request received from ${
                      newJr.name ||
                      newJr.email
                    }!`
                  );
                ...
                if (
                  ...
                  const updatedJr: JoinRequestItem =
                    parsedData;
        ...
        if (isMounted) {
          ...
          console.warn(
            '[SSE] Connection closed. Polling fallback remains active.'
          );
        }
      } catch (err: any) {
        if (err?.name !== 'AbortError') {
          console.warn(
            '[SSE] Connection unavailable. Polling fallback remains active.'
          );
        }
  ...
  const counts = useMemo(() => {
    ...
    const pending = allJoinRequests.filter(
      (jr) => jr.status === 'PENDING'
    ).length;

    const approved = allJoinRequests.filter(
      (jr) => jr.status === 'APPROVED'
    ).length;

    const rejected = allJoinRequests.filter(
      (jr) => jr.status === 'REJECTED'
    ).length;
  ...
  const displayedRequests = useMemo(() => {
    ...
    return allJoinRequests.filter(
      (jr) => jr.status === activeTab
    );
  ...
  const handleUpdateCompany = async (
    ...
    try {
      ...
      await fetchCompanySettings(false);
    } catch (err: any) {
      error(
        err.response?.data?.message ||
  ...
  const handleGenerateJoinCode = async () => {
    ...
    try {
      ...
      setCompany((prev: any) => ({
        ...
        joinCodeActive:
          res.data.joinCodeActive,
        joinCodeGeneratedAt:
          res.data.joinCodeGeneratedAt,
      }));
  ...
  const handleDeactivateJoinCode = async () => {
    ...
    try {
      const res = await api.post(
        '/companies/join-code/deactivate'
      );

      success(
        res.data.message ||
          'Join code deactivated.'
      );
      ...
      error(
        err.response?.data?.message ||
          'Failed to deactivate join code'
      );
    } finally {
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-zinc-400 mt-1">
              Manage company profile, team join
              codes, and real-time employee join
              requests.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5 self-start sm:self-center">
          <div
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-xs font-semibold ${
              isSSEConnected
                ? 'bg-emerald-50 dark:bg-zinc-800/80 border-emerald-200 dark:border-zinc-700 text-emerald-700 dark:text-emerald-400'
                : 'bg-amber-50 dark:bg-zinc-800/80 border-amber-200 dark:border-zinc-700 text-amber-700 dark:text-amber-400'
            }`}
          >
            <span
        ...
        <CardHeader>
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              ...
              When they submit it on the Join
              Company page, their request will
      ...
      <Card>
        <CardHeader className="flex flex-col sm:flex-row sm:items-center justify-between gap-3.5">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              Review and approve employees
              who submit your company join code.
            ...
            <div className="flex flex-col items-center justify-center p-12 gap-3 text-center">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-slate-400 dark:text-slate-500 max-w-sm">
                  Share your active join code
                  with team members so they can
                      ...
                      (jr) => (
                        <tr
                          ...
                          className={`hover:bg-slate-50/70 dark:hover:bg-slate-800/40 transition-colors ${
                            jr.isNew
                              ? 'bg-blue-50/40 dark:bg-blue-950/20 animate-pulse-slow'
                              : ''
                          }`}
                        >
                          {/* Applicant */}
